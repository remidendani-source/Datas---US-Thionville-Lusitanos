...
if mode == "⚽ Collectif":
    ...
    # Garde seulement les matchs où l'adversaire existe dans toutes les tables
    valid_idx = [i for i in range(len(thi_gen))
                 if len(adv_gen_frames[i])>0 and len(adv_def_frames[i])>0]
    thi_gen   = thi_gen.iloc[valid_idx].reset_index(drop=True)
    adv_gen   = pd.concat([adv_gen_frames[i]  for i in valid_idx], ignore_index=True)
    adv_off   = pd.concat([adv_off_frames[i]  for i in valid_idx], ignore_index=True)
    adv_def   = pd.concat([adv_def_frames[i]  for i in valid_idx], ignore_index=True)
    adv_pass  = pd.concat([adv_pass_frames[i] for i in valid_idx], ignore_index=True)
    adv_idx   = pd.concat([adv_idx_frames[i]  for i in valid_idx], ignore_index=True)
    adv_names = [adv_gen_frames[i]["Team"].values[0] for i in valid_idx]
