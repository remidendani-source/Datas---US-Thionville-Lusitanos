...
if mode == "⚽ Collectif":
    ...
    with _ctrl_cols[0]:
        joueur_A = st.selectbox("👤 Joueur A", ALL_PLAYERS, key="cmp_A", label_visibility="visible")
    with _ctrl_cols[1]:
        joueur_B = st.selectbox("👤 Joueur B", ALL_PLAYERS, index=1, key="cmp_B", label_visibility="visible")
    with _ctrl_cols[2]:
    ...
    match_labels_cmp = [str(r["Journée"]) for _,r in df_cmp_A_all.iterrows()]
    with _ctrl_cols[3]:
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    df_A = _get_df_cmp(joueur_A, selected_cmp)
    df_B = _get_df_cmp(joueur_B, selected_cmp)
    ...
    section("Duels aériens gagnés (%) par journée")
    fa=go.Figure()
    fa.add_trace(go.Scatter(x=_j_all,y=_yA_aer,name=joueur_A.split()[-1],mode="lines+markers+text",
    ...
    fa.add_trace(go.Scatter(x=_j_all,y=_yB_aer,name=joueur_B.split()[-1],mode="lines+markers+text",
    ...
    section("Duels défensifs gagnés (%) par journée")
    fd=go.Figure()
    fd.add_trace(go.Scatter(x=_j_all,y=_yA_def,name=joueur_A.split()[-1],mode="lines+markers+text",
        line=dict(color=COL_A,width=2),
        marker=dict(size=9,color=COL_A,line=dict(color=CARD,width=2)),
        text=[f"{v}%" if v is not None else "" for v in _yA_def],
        textposition="top center",textfont=dict(size=8,color=COL_A),
        fill="tozeroy",fillcolor=f"rgba(74,144,255,0.06)",connectgaps=True))
    fd.add_trace(go.Scatter(x=_j_all,y=_yB_def,name=joueur_B.split()[-1],mode="lines+markers+text",
        line=dict(color=COL_B,width=2),
        marker=dict(size=9,color=COL_B,line=dict(color=CARD,width=2)),
