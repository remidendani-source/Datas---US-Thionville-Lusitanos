...
ALL_PLAYERS = [
    ...
    # Nouveaux L3 2026-27
    "Bourama Diarra","Joseph Atangana","Enzo Montet","Kylian Tubio","Gauthier Laurens","Fadel Gobitaka","Simon Kalambayi"
]

@st.cache_data(ttl=300)
def load_data(path):
    # Mapping colonnes françaises (format L3) → colonnes anglaises (format N2)
    COL_MAP = {
        "Compétition": "Competition", "Championnat": "Duration",
        "Équipe": "Team", "Projet": "Scheme", "Buts": "Goals",
        "Possession, %": "% Possession",
        "Pertes/Bas/Moyen/Élevé": "Losses",
        "Récupérations/Bas/Moyen/Élevé": "Recoveries",
        "Duels / gagnés": "Duels",
    }
    def normalize_cols(df):
        ...
        for i, c in enumerate(cols):
            if c == "Tirs / cadrés":
                cols[i] = "Shots"
                if i+1 < len(cols) and "Unnamed" in str(cols[i+1]): cols[i+1] = "Shots on target"
            if c == "Passes / précises":
                cols[i] = "Passes"
                if i+1 < len(cols) and "Unnamed" in str(cols[i+1]): cols[i+1] = "Passes accurate"
                if i+2 < len(cols) and "Unnamed" in str(cols[i+2]): cols[i+2] = "% Passes accurate"
            if c == "Losses":
                if i+1 < len(cols) and "Unnamed" in str(cols[i+1]): cols[i+1] = "Losses Low "
                if i+2 < len(cols) and "Unnamed" in str(cols[i+2]): cols[i+2] = "Losses Medium"
                if i+3 < len(cols) and "Unnamed" in str(cols[i+3]): cols[i+3] = "Losses High"
            if c == "Recoveries":
                if i+1 < len(cols) and "Unnamed" in str(cols[i+1]): cols[i+1] = "Recoveries Low"
                if i+2 < len(cols) and "Unnamed" in str(cols[i+2]): cols[i+2] = "Recoveries Medium"
                if i+3 < len(cols) and "Unnamed" in str(cols[i+3]): cols[i+3] = "Recoveries High"
            if c == "Duels":
                if i+1 < len(cols) and "Unnamed" in str(cols[i+1]): cols[i+1] = "Duels won"
                if i+2 < len(cols) and "Unnamed" in str(cols[i+2]): cols[i+2] = "% Duels won"
        df.columns = cols
        return df
    ...
    def sheet_pair(base):
        ...
        for suffix in ["N2", "L3"]:
            try:
                raw = pd.read_excel(path, sheet_name=f"Stats {base} {suffix}")
                # Format L3 français : colonnes avec accents
                if "Équipe" in raw.columns or "Compétition" in raw.columns:
                    df = raw.iloc[2:].copy().reset_index(drop=True)
                    df = normalize_cols(df)
                else:
                    df = raw.iloc[2:].copy().reset_index(drop=True)
                df["Date"] = pd.to_datetime(df["Date"], errors="coerce")
                df["Match"] = df["Match"].ffill()
                df["Competition"] = df["Competition"].ffill()
        ...
        if not frames:
            try:
                raw = pd.read_excel(path, sheet_name=f"Stats {base}")
                df = raw.iloc[2:].copy().reset_index(drop=True)
                df["Date"] = pd.to_datetime(df["Date"], errors="coerce")
                df["Match"] = df["Match"].ffill()
                df["Competition"] = df["Competition"].ffill()
        ...
        aligned = [f[[c for c in cols if c in f.columns]] for f in frames]
        return pd.concat(aligned, ignore_index=True)
    return (sheet_pair("gen"), sheet_pair("off"), sheet_pair("def"),
            sheet_pair("pass"), sheet_pair("index"))
...
ALL_PLAYERS = [
    ...
    "Bryan Labissière","Alexis Gouletquer","Karim Bouhmidi","Chafik Gourichy",
    # Nouveaux L3 2026-27
    "Bourama Diarra","Joseph Atangana","Enzo Montet","Kylian Tubio","Gauthier Laurens","Simon Kalambayi"
]
COMP_LABELS = {
    "France. National 2": "National 2 · 2025-26",
    "France. Ligue 3":    "Ligue 3 · 2026-27",
...
with st.sidebar:
    ...
    # Items nav
    _modes  = ["⚽ Collectif", "👤 Joueur", "⚖️ Comparaison", "📊 KPI'S LIGUE 3"]
    _labels = ["Analyse collective", "Analyse individuelle", "Comparaison", "Indicateurs L3"]
    _icons  = ["👥", "👤", "⚖️", "📊"]
...
if mode == "⚽ Collectif":
    ...
    _ctrl_cols = st.columns([2, 2, 2, 4])
    with _ctrl_cols[0]:
        joueur_A = st.selectbox("👤 Joueur A", ["— Sélectionner —"] + ALL_PLAYERS, key="cmp_A", label_visibility="visible")
    with _ctrl_cols[1]:
        joueur_B = st.selectbox("👤 Joueur B", ["— Sélectionner —"] + ALL_PLAYERS, key="cmp_B", label_visibility="visible")
    with _ctrl_cols[2]:
        saison_label_cmp = st.selectbox("🏆 Saison", list(comp_opts.keys()), key="cmp_saison", label_visibility="visible")

    if joueur_A == "— Sélectionner —" or joueur_B == "— Sélectionner —":
        st.markdown(f'<div style="display:flex;align-items:center;justify-content:center;height:300px;color:{MUTED};font-size:15px;">👆 Sélectionnez deux joueurs pour lancer la comparaison</div>', unsafe_allow_html=True)
    else:
        # Adversaires pour le filtre
        _j_to_adv_cmp = {}
        for _, _row in gen_all[gen_all["Team"]==TEAM].iterrows():
            _j = str(_row["Journée"])
            _adv_rows = gen_all[(gen_all["Match"]==_row["Match"])&(gen_all["Team"]!=TEAM)]
            if not _adv_rows.empty:
                _j_to_adv_cmp[_j] = _adv_rows.iloc[0]["Team"]
        _comp_cmp = comp_opts[saison_label_cmp]
        df_cmp_A_all = players_all[joueur_A].copy()
        df_cmp_A_all = df_cmp_A_all[(df_cmp_A_all["Minutes played"]>20) & (df_cmp_A_all["Competition"]==_comp_cmp)].sort_values("Journée").reset_index(drop=True)
        match_labels_cmp = [f"{r['Journée']} — {_j_to_adv_cmp.get(str(r['Journée']),'?')}" for _,r in df_cmp_A_all.iterrows()]
        _cmp_label_to_j = {lbl: str(r["Journée"]) for lbl, (_,r) in zip(match_labels_cmp, df_cmp_A_all.iterrows())}
        with _ctrl_cols[3]:
            selected_cmp = st.multiselect("🗓️ Filtrer les matchs", match_labels_cmp, default=[],
                                          placeholder="Sélectionnez un/des match(s)", key="ms_cmp", label_visibility="visible")
        selected_cmp_j = [_cmp_label_to_j[l] for l in selected_cmp]
        st.markdown('<hr style="border:none;border-top:1px solid #1e2733;margin:6px 0 10px 0;"/>', unsafe_allow_html=True)

elif mode == "👤 Joueur":
    _ctrl_cols = st.columns([2,3,5])
    with _ctrl_cols[0]:
        saison_label_j = st.selectbox("🏆 Saison", list(comp_opts.keys()), label_visibility="visible")
    _comp_j = comp_opts[saison_label_j]
    # Filtre les joueurs ayant joué dans la saison sélectionnée
    _joueurs_saison = [j for j in ALL_PLAYERS
                       if j in players_all and
                       len(players_all[j][(players_all[j]["Minutes played"]>0) &
                                          (players_all[j]["Competition"]==_comp_j)]) > 0]
    if not _joueurs_saison:
        _joueurs_saison = ALL_PLAYERS
    with _ctrl_cols[1]:
        joueur_sel = st.selectbox("👤 Joueur", _joueurs_saison, label_visibility="visible")
    df_j = players_all[joueur_sel].copy()
    # Filtre par saison sélectionnée
    _comp_j = comp_opts[saison_label_j]
    df_played_all = df_j[(df_j["Minutes played"]>20) & (df_j["Competition"]==_comp_j)].sort_values("Journée").reset_index(drop=True)
    # Dictionnaire Journée → adversaire (filtré par compétition)
    _j_to_adv = {}
    for _, _row in gen_all[(gen_all["Team"]==TEAM)&(gen_all["Competition"]==_comp_j)].iterrows():
        _j = str(_row["Journée"])
        _match = _row["Match"]
        _adv_r = gen_all[(gen_all["Match"]==_match)&(gen_all["Team"]!=TEAM)&gen_all["Team"].notna()]
        if not _adv_r.empty:
            _j_to_adv[_j] = _adv_r.iloc[0]["Team"]
    match_labels_j = [f"{r['Journée']} — {_j_to_adv.get(str(r['Journée']),'?')}" for _,r in df_played_all.iterrows()]
    _label_to_j = {lbl: str(r["Journée"]) for lbl, (_,r) in zip(match_labels_j, df_played_all.iterrows())}
    with _ctrl_cols[2]:
    ...
    if selected_j:
        _sel_j_nums = [_label_to_j[l] for l in selected_j]
        df_joueur = df_played_all[df_played_all["Journée"].astype(str).isin(_sel_j_nums)].reset_index(drop=True)
    else:
...
if mode == "⚽ Collectif":
    ...
    thi_gen=gen[gen["Team"]==TEAM].dropna(subset=["Match","Journée"]).sort_values("Journée").reset_index(drop=True)
    ...
    def _adv_rows(df, match):
        return df[(df["Match"]==match) & (df["Team"]!=TEAM) & df["Team"].notna()]

    adv_gen_frames = [_adv_rows(gen, thi_gen.iloc[i]["Match"]) for i in range(len(thi_gen))]
    valid_idx = [i for i in range(len(thi_gen)) if len(adv_gen_frames[i])>0]

    if len(valid_idx) == 0:
        st.info("ℹ️ Les données adversaires ne sont pas encore disponibles pour cette saison.")
        journees = [str(j) for j in thi_gen["Journée"]]
        journees_adv = journees
        adv_gen = adv_off = adv_def = adv_pass = adv_idx = pd.DataFrame()
        adv_names = ["?" for _ in range(len(thi_gen))]
        resultats = ["N" for _ in range(len(thi_gen))]
        bar_colors = [GOLD for _ in range(len(thi_gen))]
        buts_adv_list = [0 for _ in range(len(thi_gen))]
    else:
        thi_gen = thi_gen.iloc[valid_idx].reset_index(drop=True)
        thi_off = thi_off.iloc[[i for i,_ in enumerate(thi_off.iterrows()) if True]].reset_index(drop=True)

        def safe_concat(frames):
            parts = [f for f in frames if len(f)>0]
            return pd.concat(parts, ignore_index=True) if parts else pd.DataFrame()

        adv_gen  = safe_concat([_adv_rows(gen,   thi_gen.iloc[i]["Match"]) for i in range(len(thi_gen))])
        adv_off  = safe_concat([_adv_rows(off,   thi_gen.iloc[i]["Match"]) for i in range(len(thi_gen))])
        adv_def  = safe_concat([_adv_rows(defe,  thi_gen.iloc[i]["Match"]) for i in range(len(thi_gen))])
        adv_pass = safe_concat([_adv_rows(passe, thi_gen.iloc[i]["Match"]) for i in range(len(thi_gen))])
        adv_idx  = safe_concat([_adv_rows(idx,   thi_gen.iloc[i]["Match"]) for i in range(len(thi_gen))])

        adv_names    = [adv_gen_frames[valid_idx[i]]["Team"].values[0] for i in range(len(thi_gen))]
        journees     = [str(j) for j in thi_gen["Journée"]]
        journees_adv = [f"{journees[i]}\n{adv_names[i]}" for i in range(len(thi_gen))]
        resultats    = [get_result(thi_gen.iloc[i], _adv_rows(adv_gen, thi_gen.iloc[i]["Match"]).iloc[0] if len(_adv_rows(adv_gen, thi_gen.iloc[i]["Match"]))>0 else thi_gen.iloc[i]) for i in range(len(thi_gen))]
        bar_colors   = [{"V":GREEN,"N":GOLD,"D":RED}[r] for r in resultats]
        buts_adv_list= [_adv_rows(adv_gen, row["Match"])["Goals"].values[0] if len(_adv_rows(adv_gen, row["Match"]))>0 else 0 for _,row in thi_gen.iterrows()]

    if len(thi_gen)==0: st.warning("Aucune donnée disponible."); st.stop()
...
if mode == "📊 KPI'S LIGUE 3":
    ...
    # ── Tableau L3 dynamique : 1 ligne par journée + total ──
    def _kpi_l3_from_data():
        try:
            def _r(sheet):
                raw = pd.read_excel(FICHIER, sheet_name=sheet)
                df  = raw.iloc[2:].copy().reset_index(drop=True)
                df["Match"]       = df["Match"].ffill()
                df["Competition"] = df["Competition"].ffill()
                if "Team" in df.columns:
                    df = df[df["Team"].notna()].reset_index(drop=True)
                return df
            _gen  = _r("Stats gen L3");  _pass = _r("Stats pass L3"); _idx = _r("Stats index L3")
            _tg   = _gen[_gen["Team"]==TEAM].copy()
            _tp   = _pass[_pass["Team"]==TEAM].copy()
            _ti   = _idx[_idx["Team"]==TEAM].copy()
            _ag   = _gen[_gen["Team"]!=TEAM].copy()

            def _kpi(tg, tp, ti, ag):
                dur = tg["Duration"].sum()
                if dur == 0: return None
                return {
                    "K1": round(ti["PPDA"].mean(), 2),
                    "K2": round(tg["Recoveries High"].sum() / dur * 90, 2),
                    "K3": round(tp["Passes to final third accurate"].sum() / dur * 90, 2),
                    "K4": round(tp["Progressive passes accurate"].sum() / dur * 90, 2),
                    "K5": round(tg["Losses Low "].sum() / tg["Losses"].sum() * 100, 1) if tg["Losses"].sum() > 0 else 0,
                    "K6": round(ag["Shots"].sum() / dur * 90, 2),
                    "K7": round(tg["Recoveries High"].sum() / tg["Losses Low "].sum(), 2) if tg["Losses Low "].sum() > 0 else 0,
                }

            rows_html = ""
            journees = sorted(_tg["Journée"].dropna().unique().tolist(),
                              key=lambda x: int(str(x).lstrip("J")) if str(x).lstrip("J").isdigit() else 0)
            for j in journees:
                tg = _tg[_tg["Journée"]==j]; tp = _tp[_tp["Match"]==tg.iloc[0]["Match"]] if len(tg) else pd.DataFrame()
                ti = _ti[_ti["Match"]==tg.iloc[0]["Match"]] if len(tg) else pd.DataFrame()
                ag = _ag[_ag["Match"]==tg.iloc[0]["Match"]] if len(tg) else pd.DataFrame()
                kv = _kpi(tg, tp, ti, ag)
                if not kv: continue
                sc = sum([kv["K1"]<=10.5, kv["K2"]>=9.0, kv["K3"]>=5.0, kv["K4"]>=65.0,
                          kv["K5"]<=16, kv["K6"]<=10.0, kv["K7"]>=0.80])
                bg, fg = score_style(sc)
                rows_html += f"""<tr style="opacity:0.8;">
                  <td style="text-align:center;"><span class="rank-badge" style="background:#0d1e3b;color:#4A90D9;font-size:9px;">{j}</span></td>
                  <td style="padding-left:12px;font-size:11px;color:#8b949e;">TL vs {_ag[_ag["Match"]==tg.iloc[0]["Match"]]["Team"].values[0] if len(_ag[_ag["Match"]==tg.iloc[0]["Match"]])>0 else "?"}</td>
                  {td_n2(kv["K1"],"k1")}{td_n2(kv["K2"],"k2")}{td_n2(kv["K3"],"k3")}{td_n2(kv["K4"],"k4")}
                  {td_n2(kv["K5"],"k5",pct=True)}{td_n2(kv["K6"],"k6")}{td_n2(kv["K7"],"k7")}
                  <td style="text-align:center;"><span class="score-pill" style="background:{bg};color:{fg};font-size:10px;">{sc}/7</span></td>
                </tr>"""

            # Ligne totale
            kv_tot = _kpi(_tg, _tp, _ti, _ag)
            if kv_tot:
                sc_tot = sum([kv_tot["K1"]<=10.5, kv_tot["K2"]>=9.0, kv_tot["K3"]>=5.0, kv_tot["K4"]>=65.0,
                              kv_tot["K5"]<=16, kv_tot["K6"]<=10.0, kv_tot["K7"]>=0.80])
                bg_tot, fg_tot = score_style(sc_tot)
                rows_html += f"""<tr style="border-top:1px solid #2a3a50;">
                  <td style="text-align:center;"><span class="rank-badge" style="background:#0d1e3b;color:#4A90D9;">TL</span></td>
                  <td style="padding-left:12px;font-weight:700;color:#C9A84C;">US Thionville Lusitanos <span style="font-size:10px;color:#8b949e;font-weight:400;">L3 26/27 — {len(journees)} J</span></td>
                  {td_n2(kv_tot["K1"],"k1")}{td_n2(kv_tot["K2"],"k2")}{td_n2(kv_tot["K3"],"k3")}{td_n2(kv_tot["K4"],"k4")}
                  {td_n2(kv_tot["K5"],"k5",pct=True)}{td_n2(kv_tot["K6"],"k6")}{td_n2(kv_tot["K7"],"k7")}
                  <td style="text-align:center;"><span class="score-pill" style="background:{bg_tot};color:{fg_tot};">{sc_tot}/7</span></td>
                </tr>"""
            return rows_html
        except Exception as e:
            return f'<tr><td colspan="10" style="color:#ff4444;padding:8px;">Erreur chargement L3 : {e}</td></tr>'

    thl_l3_html = f"""
    <table class="kpi-tbl" style="margin-top:8px;">
      <tbody>
        {_kpi_l3_from_data()}
      </tbody>
    </table>
    ...
    # ── Ligne L3 sous le radar ──
    st.markdown(thl_l3_html, unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    journees_j_adv=[f"{r['Journée']}\n{_j_to_adv.get(str(r['Journée']),'')}" for _,r in df.iterrows()]
    ...
    JOUEUR_POSTES = {
        ...
        # Nouveaux L3 2026-27
        "Joseph Atangana":   [(.78,.50)],
        "Bourama Diarra":    [(.62,.85)],
        "Enzo Montet":       [(.45,.70)],
        "Kylian Tubio":      [(.12,.50)],
        "Gauthier Laurens":  [(.32,.50)],
        "Fadel Gobitaka":    [(.62,.15)],
        "Simon Kalambayi":   [(.12,.50)],
    }
    ...
    with col_p:
        ...
        POSTE_MAP = {
            ...
            "Enzo Montet":"MC","Gauthier Laurens":"MC",
            "Bryan Labissière":"ATT","Karim Bouhmidi":"ATT","Chafik Gourichy":"ATT","Alexis Gouletquer":"ATT","Simon Kalambayi":"ATT","Kylian Tubio":"ATT",
        }
    ...
    fa=go.Figure()
    fa.add_trace(go.Scatter(x=journees_j_adv,y=aer_pm,mode="lines+markers+text",
    ...
    fd=go.Figure()
    fd.add_trace(go.Scatter(x=journees_j_adv,y=def_pm,mode="lines+markers+text",
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    _joueur_A = st.session_state.get("cmp_A", "— Sélectionner —")
    _joueur_B = st.session_state.get("cmp_B", "— Sélectionner —")
    if _joueur_A == "— Sélectionner —" or _joueur_B == "— Sélectionner —":
        pass  # Message déjà affiché dans les contrôles
    else:
        joueur_A = _joueur_A; joueur_B = _joueur_B
        _saison_label_cmp2 = st.session_state.get("cmp_saison", list(comp_opts.keys())[0])
        _comp_cmp2 = comp_opts[_saison_label_cmp2]

        def _get_df_cmp(joueur, selected):
            df = players_all[joueur].copy()
            df = df[(df["Minutes played"]>20) & (df["Competition"]==_comp_cmp2)].sort_values("Journée").reset_index(drop=True)
            if selected:
                df = df[df["Journée"].isin(selected)].reset_index(drop=True)
            return df

        def _calc_cmp(df):
            C = get_player_cols(df)
            def sp(w,t): return round(w/t*100,1) if t>0 else 0
            n=len(df); mins=int(df["Minutes played"].sum())
            buts=int(df["Goals"].sum()); assists=int(df["Assists"].sum())
            yc=int(df[C["yc"]].sum()); rc=int(df[C["rc"]].sum()); mpm=round(mins/n,1) if n>0 else 0
            ta_t=df["Total actions / successful"].sum(); ta_w=df[C["ta_acc"]].sum(); ta_pct=sp(ta_w,ta_t)
            pass_t=df["Passes / accurate"].sum(); pass_w=df[C["pass_acc"]].sum(); pass_pct=sp(pass_w,pass_t)
            long_t=df["Long passes / accurate"].sum(); long_w=df[C["long_acc"]].sum(); long_pct=sp(long_w,long_t)
            fp_t=df["Forward passes / accurate"].sum(); fp_w=df[C["fp_acc"]].sum(); fp_pct=sp(fp_w,fp_t)
            ...
            tp_t=df["Through passes / accurate"].sum(); tp_w=df[C["through_acc"]].sum(); tp_pct=sp(tp_w,tp_t)
            crosses_t=df["Crosses / accurate"].sum(); crosses_w=df[C["crosses_acc"]].sum(); crosses_pct=sp(crosses_w,crosses_t)
            rec_passes=int(df["Received passes"].sum()); pass_pm=round(pass_t/n,1) if n>0 else 0
            pw=max(int(pass_w),1)
            aer_t=df["Aerial duels / won"].sum(); aer_w=df[C["aer_acc"]].sum(); aer_pct=sp(aer_w,aer_t)
            def_col="Defensive duels / won" if "Defensive duels / won" in df.columns else "Duels / won"
            def_t=df[def_col].sum(); def_w=df[C["def_acc"]].sum() if C["def_acc"] else 0; def_pct=sp(def_w,def_t)
            ...
            off_t=df["Offensive duels / won"].sum(); off_w=df[C["off_acc"]].sum(); off_pct=sp(off_w,off_t)
            drib_t=df["Dribbles / successful"].sum(); drib_w=df[C["drib_acc"]].sum(); drib_pct=sp(drib_w,drib_t)
            prog_runs=int(df["Progressive runs"].sum()); touches_pa=int(df["Touches in penalty area"].sum())
            fouls_suf=int(df["Fouls suffered"].sum())
            xg_j=round(df["xG"].sum(),2); xA_j=round(df["xA"].sum(),2)
            shots_t=df["Shots / on target"].sum(); shots_w=df[C["shots_acc"]].sum(); conv_pct=sp(buts,shots_t)
            shot_assists_j=int(df["Shot assists"].sum()); second_assists=int(df["Second assists"].sum())
            journees=[str(r["Journée"]) for _,r in df.iterrows()]
            aer_pm=[round(r[C["aer_acc"]]/r["Aerial duels / won"]*100,1) if r["Aerial duels / won"]>0 else 0 for _,r in df.iterrows()]
            _dc="Defensive duels / won" if "Defensive duels / won" in df.columns else "Duels / won"
            def_pm=[round(r[C["def_acc"]]/r[_dc]*100,1) if C["def_acc"] and r[_dc]>0 else 0 for _,r in df.iterrows()]
            return dict(n=n,mins=mins,buts=buts,assists=assists,yc=yc,rc=rc,mpm=mpm,
                        ta_t=ta_t,ta_w=ta_w,ta_pct=ta_pct,
                        pass_pct=pass_pct,pass_pm=pass_pm,pass_w=pass_w,rec_passes=rec_passes,
                        fp_w=fp_w,bp_w=bp_w,ptf_w=ptf_w,long_w=long_w,crosses_w=crosses_w,tp_w=tp_w,ppa_w=ppa_w,pw=pw,
                        fp_pct=fp_pct,bp_pct=bp_pct,ptf_pct=ptf_pct,ppa_pct=ppa_pct,long_pct=long_pct,tp_pct=tp_pct,crosses_pct=crosses_pct,
                        aer_t=aer_t,aer_w=aer_w,aer_pct=aer_pct,
                        def_t=def_t,def_w=def_w,def_pct=def_pct,
                        rec_j=rec_j,rec_j_w=rec_j_w,rec_zone_pct=rec_zone_pct,
                        loss_j=loss_j,loss_j_w=loss_j_w,loss_zone_pct=loss_zone_pct,
                        intercept_j=intercept_j,slides_t=slides_t,slides_w_n=slides_w_n,fouls_j=fouls_j,
                        off_t=off_t,off_w=off_w,off_pct=off_pct,
                        drib_t=drib_t,drib_w=drib_w,drib_pct=drib_pct,
                        prog_runs=prog_runs,touches_pa=touches_pa,fouls_suf=fouls_suf,
                        xg_j=xg_j,xA_j=xA_j,shots_t=shots_t,shots_w=shots_w,conv_pct=conv_pct,
                        shot_assists_j=shot_assists_j,second_assists=second_assists,
                        journees=journees,aer_pm=aer_pm,def_pm=def_pm)

        df_A = _get_df_cmp(joueur_A, selected_cmp_j)
        df_B = _get_df_cmp(joueur_B, selected_cmp_j)
        sA = _calc_cmp(df_A); sB = _calc_cmp(df_B)

        def cmp_mirror_row(label, vA, vB, mA, mB, pct=False, higher_is_better=True, neutral=False):
            wA=round(vA/mA*100,1) if mA>0 else 0
            wB=round(vB/mB*100,1) if mB>0 else 0
            sA_=f"{vA:.0f}%" if pct else str(round(vA,1))
            sB_=f"{vB:.0f}%" if pct else str(round(vB,1))
            if neutral:
                cA_ = "#fff"; cB_ = "#fff"
            else:
                better_A = (vA > vB) if higher_is_better else (vA < vB)
                better_B = (vB > vA) if higher_is_better else (vB < vA)
                cA_ = GREEN if better_A else (GOLD if vA==vB else "#fff")
                cB_ = GREEN if better_B else (GOLD if vA==vB else "#fff")
            r='<div style="display:grid;grid-template-columns:60px 1fr 180px 1fr 60px;align-items:center;gap:4px;padding:6px 0;border-bottom:0.5px solid #1e2733;">'
            r+=f'<div style="text-align:right;font-size:13px;font-weight:700;color:{cA_};">{sA_}</div>'
            r+=f'<div style="background:#1a2030;border-radius:3px;height:14px;overflow:hidden;"><div style="width:{wA}%;height:100%;background:{COL_A};margin-left:auto;border-radius:3px;opacity:0.85;"></div></div>'
            r+=f'<div style="text-align:center;font-size:11px;color:{MUTED};padding:0 6px;">{label}</div>'
            r+=f'<div style="background:#1a2030;border-radius:3px;height:14px;overflow:hidden;"><div style="width:{wB}%;height:100%;background:{COL_B};border-radius:3px;opacity:0.85;"></div></div>'
            r+=f'<div style="text-align:left;font-size:13px;font-weight:700;color:{cB_};">{sB_}</div></div>'
            return r

        def cmp_build(rows):
            h=f'<div style="display:grid;grid-template-columns:60px 1fr 180px 1fr 60px;gap:4px;padding-bottom:8px;border-bottom:1px solid #1e2733;margin-bottom:4px;">'
            h+=f'<div style="text-align:right;font-size:12px;font-weight:800;color:{COL_A};">{joueur_A.split()[-1]}</div>'
            h+='<div></div><div></div><div></div>'
            h+=f'<div style="text-align:left;font-size:12px;font-weight:800;color:{COL_B};">{joueur_B.split()[-1]}</div></div>'
            return f'<div style="background:#111827;border:1px solid #1e2733;border-radius:10px;padding:14px;">{h}{rows}</div>'

        def cmp_section(title, rows):
            return f'<div style="margin-bottom:12px;"><div class="sct">{title}</div>{cmp_build(rows)}</div>'

        def _photo_cmp(joueur, size=80):
            import base64 as _b64c, unicodedata as _uni
            _base = Path(__file__).parent / "photos"
            for _n in [joueur, _uni.normalize("NFC",joueur), _uni.normalize("NFD",joueur)]:
                for _e in ["png","jpg","jpeg","PNG","JPG"]:
                    _p = _base / f"{_n}.{_e}"
                    if _p.exists():
                        with open(_p,"rb") as _f: _b = _b64c.b64encode(_f.read()).decode()
                        _m = "image/jpeg" if str(_p).lower().endswith(("jpg","jpeg")) else "image/png"
                        return f'<img src="data:{_m};base64,{_b}" style="height:{size}px;width:auto;object-fit:contain;border-radius:8px;border:2px solid rgba(201,168,76,0.35);"/>'
            return f'<div style="height:{size}px;width:{size}px;background:#1e2733;border-radius:8px;display:flex;align-items:center;justify-content:center;font-size:28px;">👤</div>'

        hA=f'''<div style="background:#0d1117;border:2px solid {COL_A}44;border-radius:10px;padding:12px 16px;display:flex;align-items:center;gap:12px;">
            {_photo_cmp(joueur_A)}
            <div><div style="font-size:18px;font-weight:900;color:{COL_A};">{joueur_A.upper()}</div>
            <div style="font-size:11px;color:{MUTED};">{sA["n"]} matchs · {sA["mins"]} min · {sA["mpm"]} min/m</div>
            <div style="margin-top:6px;height:4px;background:#1a2030;border-radius:2px;width:140px;"><div style="width:{sA["ta_pct"]}%;height:100%;background:{COL_A};border-radius:2px;"></div></div>
            <div style="font-size:9px;color:{MUTED};margin-top:2px;">Actions réussies : {sA["ta_pct"]}%</div></div></div>'''
        hB=f'''<div style="background:#0d1117;border:2px solid {COL_B}44;border-radius:10px;padding:12px 16px;display:flex;align-items:center;gap:12px;justify-content:flex-end;">
            <div style="text-align:right;"><div style="font-size:18px;font-weight:900;color:{COL_B};">{joueur_B.upper()}</div>
            <div style="font-size:11px;color:{MUTED};">{sB["n"]} matchs · {sB["mins"]} min · {sB["mpm"]} min/m</div>
            <div style="margin-top:6px;height:4px;background:#1a2030;border-radius:2px;width:140px;margin-left:auto;"><div style="width:{sB["ta_pct"]}%;height:100%;background:{COL_B};border-radius:2px;"></div></div>
            <div style="font-size:9px;color:{MUTED};margin-top:2px;">Actions réussies : {sB["ta_pct"]}%</div></div>
            {_photo_cmp(joueur_B)}</div>'''

        cA_h,cVS,cB_h=st.columns([5,1,5])
        with cA_h: st.markdown(hA,unsafe_allow_html=True)
        with cVS: st.markdown(f'<div style="display:flex;align-items:center;justify-content:center;height:100%;font-size:20px;font-weight:900;color:{GOLD};">VS</div>',unsafe_allow_html=True)
        with cB_h: st.markdown(hB,unsafe_allow_html=True)
        st.markdown("<div style='height:8px'></div>",unsafe_allow_html=True)

        # ── Bandeau Total Actions (comme Analyse individuelle) ──
        _bA,_bB=st.columns(2)
        with _bA:
            st.markdown(f"""<div style="background:#1E2733;border:1px solid rgba(201,168,76,0.3);border-radius:10px;padding:10px 16px;display:flex;align-items:center;gap:20px;flex-wrap:wrap;">
              <div style="text-align:center;"><div style="font-size:9px;color:{MUTED};text-transform:uppercase;">Total actions</div><div style="font-size:22px;font-weight:900;color:#ffffff;">{int(sA["ta_t"])}</div></div>
              <div style="width:1px;height:28px;background:{BORD};"></div>
              <div style="text-align:center;"><div style="font-size:9px;color:{MUTED};text-transform:uppercase;">Réussies</div><div style="font-size:22px;font-weight:900;color:{GREEN};">{int(sA["ta_w"])}</div></div>
              <div style="width:1px;height:28px;background:{BORD};"></div>
              <div style="text-align:center;"><div style="font-size:9px;color:{MUTED};text-transform:uppercase;">% Réussite</div><div style="font-size:22px;font-weight:900;color:{GOLD};">{sA["ta_pct"]}%</div></div>
              <div style="flex:1;min-width:60px;"><div style="height:6px;background:{BORD};border-radius:3px;overflow:hidden;"><div style="width:{sA["ta_pct"]}%;height:100%;background:linear-gradient(90deg,{GREEN},{GOLD});border-radius:3px;"></div></div></div>
            </div>""", unsafe_allow_html=True)
        with _bB:
            st.markdown(f"""<div style="background:#1E2733;border:1px solid rgba(201,168,76,0.3);border-radius:10px;padding:10px 16px;display:flex;align-items:center;gap:20px;flex-wrap:wrap;">
              <div style="text-align:center;"><div style="font-size:9px;color:{MUTED};text-transform:uppercase;">Total actions</div><div style="font-size:22px;font-weight:900;color:#ffffff;">{int(sB["ta_t"])}</div></div>
              <div style="width:1px;height:28px;background:{BORD};"></div>
              <div style="text-align:center;"><div style="font-size:9px;color:{MUTED};text-transform:uppercase;">Réussies</div><div style="font-size:22px;font-weight:900;color:{GREEN};">{int(sB["ta_w"])}</div></div>
              <div style="width:1px;height:28px;background:{BORD};"></div>
              <div style="text-align:center;"><div style="font-size:9px;color:{MUTED};text-transform:uppercase;">% Réussite</div><div style="font-size:22px;font-weight:900;color:{GOLD};">{sB["ta_pct"]}%</div></div>
              <div style="flex:1;min-width:60px;"><div style="height:6px;background:{BORD};border-radius:3px;overflow:hidden;"><div style="width:{sB["ta_pct"]}%;height:100%;background:linear-gradient(90deg,{GREEN},{GOLD});border-radius:3px;"></div></div></div>
            </div>""", unsafe_allow_html=True)
        ...
        with c1:
            r=""
            mx_b=max(sA["buts"],sB["buts"],1); mx_a=max(sA["assists"],sB["assists"],1)
            mx_xg=max(sA["xg_j"],sB["xg_j"],0.1); mx_xA=max(sA["xA_j"],sB["xA_j"],0.1)
            mx_sh=max(sA["shot_assists_j"],sB["shot_assists_j"],1)
            r+=cmp_mirror_row("Buts",sA["buts"],sB["buts"],mx_b,mx_b)
            r+=cmp_mirror_row("xG",sA["xg_j"],sB["xg_j"],mx_xg,mx_xg)
            r+=cmp_mirror_row("Taux conv. %",sA["conv_pct"],sB["conv_pct"],100,100,pct=True)
            r+=cmp_mirror_row("Passes déc.",sA["assists"],sB["assists"],mx_a,mx_a)
            r+=cmp_mirror_row("xA",sA["xA_j"],sB["xA_j"],mx_xA,mx_xA)
            r+=cmp_mirror_row("Passes clés",sA["shot_assists_j"],sB["shot_assists_j"],mx_sh,mx_sh)
            st.markdown(cmp_section("⚽ Finition & Création",r),unsafe_allow_html=True)

            r=""
            mx_pm=max(sA["pass_pm"],sB["pass_pm"],1); mx_pw=max(sA["pass_w"],sB["pass_w"],1)
            r+=cmp_mirror_row("Passes / match",sA["pass_pm"],sB["pass_pm"],mx_pm,mx_pm,neutral=True)
            r+=cmp_mirror_row("Passes réussies",sA["pass_w"],sB["pass_w"],mx_pw,mx_pw)
            r+=cmp_mirror_row("Précision %",sA["pass_pct"],sB["pass_pct"],100,100,pct=True)
            r+=cmp_mirror_row("Passes reçues",sA["rec_passes"],sB["rec_passes"],max(sA["rec_passes"],sB["rec_passes"],1),max(sA["rec_passes"],sB["rec_passes"],1),neutral=True)
            r+=cmp_mirror_row("Avant réussies",sA["fp_w"],sB["fp_w"],mx_pw,mx_pw)
            r+=cmp_mirror_row("Précision avant %",sA["fp_pct"],sB["fp_pct"],100,100,pct=True)
            r+=cmp_mirror_row("Dernier tiers réussies",sA["ptf_w"],sB["ptf_w"],mx_pw,mx_pw)
            r+=cmp_mirror_row("Précision dernier tiers %",sA["ptf_pct"],sB["ptf_pct"],100,100,pct=True)
            r+=cmp_mirror_row("Longues réussies",sA["long_w"],sB["long_w"],mx_pw,mx_pw)
            r+=cmp_mirror_row("Précision longues %",sA["long_pct"],sB["long_pct"],100,100,pct=True)
            r+=cmp_mirror_row("Profondeur %",sA["tp_pct"],sB["tp_pct"],100,100,pct=True)
            r+=cmp_mirror_row("Centres réussis",sA["crosses_w"],sB["crosses_w"],max(sA["crosses_w"],sB["crosses_w"],1),max(sA["crosses_w"],sB["crosses_w"],1))
            st.markdown(cmp_section("🏗️ Construction & Passes",r),unsafe_allow_html=True)

        with c2:
            r=""
            mx_at=max(sA["aer_t"],sB["aer_t"],1); mx_dt=max(sA["def_t"],sB["def_t"],1)
            mx_int=max(sA["intercept_j"],sB["intercept_j"],1)
            r+=cmp_mirror_row("Duels aériens disp.",sA["aer_t"],sB["aer_t"],mx_at,mx_at,neutral=True)
            r+=cmp_mirror_row("Duels aériens gagnés %",sA["aer_pct"],sB["aer_pct"],100,100,pct=True)
            r+=cmp_mirror_row("Duels déf. disputés",sA["def_t"],sB["def_t"],mx_dt,mx_dt,neutral=True)
            r+=cmp_mirror_row("Duels déf. gagnés %",sA["def_pct"],sB["def_pct"],100,100,pct=True)
            r+=cmp_mirror_row("Récup. zone off.",sA["rec_zone_pct"],sB["rec_zone_pct"],100,100,pct=True)
            r+=cmp_mirror_row("Pertes zone déf.",sA["loss_zone_pct"],sB["loss_zone_pct"],100,100,pct=True,higher_is_better=False)
            r+=cmp_mirror_row("Interceptions",sA["intercept_j"],sB["intercept_j"],mx_int,mx_int)
            r+=cmp_mirror_row("Tacles glissés réussis",sA["slides_w_n"],sB["slides_w_n"],max(sA["slides_w_n"],sB["slides_w_n"],1),max(sA["slides_w_n"],sB["slides_w_n"],1))
            r+=cmp_mirror_row("Fautes commises",sA["fouls_j"],sB["fouls_j"],max(sA["fouls_j"],sB["fouls_j"],1),max(sA["fouls_j"],sB["fouls_j"],1),higher_is_better=False)
            st.markdown(cmp_section("🛡️ Défense",r),unsafe_allow_html=True)

            r=""
            mx_ot=max(sA["off_t"],sB["off_t"],1); mx_dt2=max(sA["drib_t"],sB["drib_t"],1)
            mx_pr=max(sA["prog_runs"],sB["prog_runs"],1); mx_tp=max(sA["touches_pa"],sB["touches_pa"],1)
            r+=cmp_mirror_row("Duels off. disputés",sA["off_t"],sB["off_t"],mx_ot,mx_ot,neutral=True)
            r+=cmp_mirror_row("Duels off. gagnés %",sA["off_pct"],sB["off_pct"],100,100,pct=True)
            r+=cmp_mirror_row("Dribbles tentés",sA["drib_t"],sB["drib_t"],mx_dt2,mx_dt2,neutral=True)
            r+=cmp_mirror_row("Dribbles réussis %",sA["drib_pct"],sB["drib_pct"],100,100,pct=True)
            r+=cmp_mirror_row("Courses progressives",sA["prog_runs"],sB["prog_runs"],mx_pr,mx_pr)
            r+=cmp_mirror_row("Touches surface adv.",sA["touches_pa"],sB["touches_pa"],mx_tp,mx_tp)
            r+=cmp_mirror_row("Fautes subies",sA["fouls_suf"],sB["fouls_suf"],max(sA["fouls_suf"],sB["fouls_suf"],1),max(sA["fouls_suf"],sB["fouls_suf"],1))
            st.markdown(cmp_section("🚀 Offensif",r),unsafe_allow_html=True)

        # ── Direction des passes côte à côte ──
        st.markdown("<div style='height:8px'></div>",unsafe_allow_html=True)
        POSTE_MAP_CMP = {
            "Muamer Aljic":"DC","Samir Bouzar":"DC","Cachito Wanduka":"DC",
            "Marly Rampont":"DC","David Luvualu":"DC",
        ...
        col_svgA, col_svgB = st.columns(2)
        with col_svgA:
            section(f"Direction des passes — {joueur_A.split()[-1]}")
            _pA = POSTE_MAP_CMP.get(joueur_A,"DC")
            _pw_A = max(int(sA["pass_w"]),1)
            _fp_rA = round(sA["fp_w"]/_pw_A*100,1); _bp_rA = round(sA["bp_w"]/_pw_A*100,1)
            _long_rA = round(sA["long_w"]/_pw_A*100,1); _ptf_rA = round(sA["ptf_w"]/_pw_A*100,1)
            _ppa_rA = round(sA["ppa_w"]/_pw_A*100,1); _tp_rA = round(sA["tp_w"]/_pw_A*100,1)
            svg_A = make_pass_svg(_fp_rA,_bp_rA,_long_rA,0,_ptf_rA,_ppa_rA,_pw_A,
                                  width=440,height=330,poste=_pA,
                                  long_pct=sA["long_pct"],ptf_pct=sA["ptf_pct"],
                                  ppa_pct=sA["ppa_pct"],crosses_pct=sA["crosses_pct"])
            st.markdown(f'<div style="background:#0d1117;border:1px solid rgba(255,255,255,0.07);border-radius:10px;padding:8px;">{svg_A}</div>',unsafe_allow_html=True)
        with col_svgB:
            section(f"Direction des passes — {joueur_B.split()[-1]}")
            _pB = POSTE_MAP_CMP.get(joueur_B,"DC")
            _pw_B = max(int(sB["pass_w"]),1)
            _fp_rB = round(sB["fp_w"]/_pw_B*100,1); _bp_rB = round(sB["bp_w"]/_pw_B*100,1)
            _long_rB = round(sB["long_w"]/_pw_B*100,1); _ptf_rB = round(sB["ptf_w"]/_pw_B*100,1)
            _ppa_rB = round(sB["ppa_w"]/_pw_B*100,1); _tp_rB = round(sB["tp_w"]/_pw_B*100,1)
            svg_B = make_pass_svg(_fp_rB,_bp_rB,_long_rB,0,_ptf_rB,_ppa_rB,_pw_B,
                                  width=440,height=330,poste=_pB,
                                  long_pct=sB["long_pct"],ptf_pct=sB["ptf_pct"],
                                  ppa_pct=sB["ppa_pct"],crosses_pct=sB["crosses_pct"])
            st.markdown(f'<div style="background:#0d1117;border:1px solid rgba(255,255,255,0.07);border-radius:10px;padding:8px;">{svg_B}</div>',unsafe_allow_html=True)

        # ── Graphiques ligne+fill comme Analyse individuelle ──
        st.markdown("<div style='height:8px'></div>",unsafe_allow_html=True)
        _j_all=sorted(set(sA["journees"])|set(sB["journees"]),key=lambda x:int(x.lstrip("J")) if x.lstrip("J").isdigit() else 0)
        _j_labels=[f"{j}\n{_j_to_adv_cmp.get(j,'')}" for j in _j_all]
        _aer_A={j:v for j,v in zip(sA["journees"],sA["aer_pm"])}
        _aer_B={j:v for j,v in zip(sB["journees"],sB["aer_pm"])}
        _def_A={j:v for j,v in zip(sA["journees"],sA["def_pm"])}
        _def_B={j:v for j,v in zip(sB["journees"],sB["def_pm"])}

        _yA_aer=[_aer_A.get(j,None) for j in _j_all]
        _yB_aer=[_aer_B.get(j,None) for j in _j_all]
        _yA_def=[_def_A.get(j,None) for j in _j_all]
        _yB_def=[_def_B.get(j,None) for j in _j_all]

        section("Duels aériens gagnés (%) par journée")
        fa=go.Figure()
        fa.add_trace(go.Scatter(x=_j_labels,y=_yA_aer,name=joueur_A.split()[-1],mode="lines+markers+text",
            line=dict(color=COL_A,width=2),
            marker=dict(size=9,color=COL_A,line=dict(color=CARD,width=2)),
            text=[f"{v}%" if v is not None else "" for v in _yA_aer],
            textposition="top center",textfont=dict(size=8,color=COL_A),
            fill="tozeroy",fillcolor=f"rgba(74,144,255,0.06)",connectgaps=True))
        fa.add_trace(go.Scatter(x=_j_labels,y=_yB_aer,name=joueur_B.split()[-1],mode="lines+markers+text",
            line=dict(color=COL_B,width=2),
            marker=dict(size=9,color=COL_B,line=dict(color=CARD,width=2)),
            text=[f"{v}%" if v is not None else "" for v in _yB_aer],
            textposition="bottom center",textfont=dict(size=8,color=COL_B),
            fill="tozeroy",fillcolor=f"rgba(245,166,35,0.06)",connectgaps=True))
        fa.add_hline(y=sA["aer_pct"],line_dash="dot",line_color=COL_A,
            annotation_text=f"Moy. {joueur_A.split()[-1]} {sA['aer_pct']:.0f}%",annotation_font_color=COL_A)
        fa.add_hline(y=sB["aer_pct"],line_dash="dot",line_color=COL_B,
            annotation_text=f"Moy. {joueur_B.split()[-1]} {sB['aer_pct']:.0f}%",annotation_font_color=COL_B)
        fa.update_layout(**lyt(),height=370); fa.update_yaxes(range=[0,125],gridcolor=BORD)
        st.plotly_chart(fa,use_container_width=True)

        section("Duels défensifs gagnés (%) par journée")
        fd=go.Figure()
        fd.add_trace(go.Scatter(x=_j_labels,y=_yA_def,name=joueur_A.split()[-1],mode="lines+markers+text",
            line=dict(color=COL_A,width=2),
            marker=dict(size=9,color=COL_A,line=dict(color=CARD,width=2)),
            text=[f"{v}%" if v is not None else "" for v in _yA_def],
            textposition="top center",textfont=dict(size=8,color=COL_A),
            fill="tozeroy",fillcolor=f"rgba(74,144,255,0.06)",connectgaps=True))
        fd.add_trace(go.Scatter(x=_j_labels,y=_yB_def,name=joueur_B.split()[-1],mode="lines+markers+text",
            line=dict(color=COL_B,width=2),
            marker=dict(size=9,color=COL_B,line=dict(color=CARD,width=2)),
            text=[f"{v}%" if v is not None else "" for v in _yB_def],
            textposition="bottom center",textfont=dict(size=8,color=COL_B),
            fill="tozeroy",fillcolor=f"rgba(245,166,35,0.06)",connectgaps=True))
        fd.add_hline(y=sA["def_pct"],line_dash="dot",line_color=COL_A,
            annotation_text=f"Moy. {joueur_A.split()[-1]} {sA['def_pct']:.0f}%",annotation_font_color=COL_A)
        fd.add_hline(y=sB["def_pct"],line_dash="dot",line_color=COL_B,
            annotation_text=f"Moy. {joueur_B.split()[-1]} {sB['def_pct']:.0f}%",annotation_font_color=COL_B)
        fd.update_layout(**lyt(),height=370); fd.update_yaxes(range=[0,125],gridcolor=BORD)
        st.plotly_chart(fd,use_container_width=True)
