...
ALL_PLAYERS = [
    ...
    # Nouveaux L3 2026-27
    "Bourama Diarra","Jérémy Atangana","Emmanuel Montet","Kyllian Tubio","Guillaume Laurens"
]
    ...
    def sheet_pair(base):
        ...
        for suffix in ["N2", "L3"]:
            try:
                ...
                # Propage Match et Competition sur les lignes adversaires (Journée=NaN)
                df["Match"]       = df["Match"].ffill()
                df["Competition"] = df["Competition"].ffill()
                frames.append(df)
            except Exception:
                pass
        # Fallback : ancien nom de feuille sans suffixe
        if not frames:
            try:
...
if mode == "⚽ Collectif":
    ...
    # Dictionnaire Journée → adversaire
    _j_to_adv = {}
    for _, _row in gen_all[gen_all["Team"]==TEAM].iterrows():
        _j = str(_row["Journée"])
        _match = _row["Match"]
        _adv_rows = gen_all[(gen_all["Match"]==_match)&(gen_all["Team"]!=TEAM)]
        if not _adv_rows.empty:
            _j_to_adv[_j] = _adv_rows.iloc[0]["Team"]
...
if mode == "⚽ Collectif":
    ...
    # Filtre les lignes valides (Match et Team non-null)
    thi_gen = thi_gen[thi_gen["Match"].notna() & thi_gen["Journée"].notna()].reset_index(drop=True)
    if len(thi_gen)==0: st.warning("Aucune donnée."); st.stop()
    def _adv_name(match):
        rows = gen[(gen["Match"]==match) & (gen["Team"]!=TEAM) & gen["Team"].notna()]
        return rows["Team"].values[0] if len(rows)>0 else "?"
    
    # Filtre les lignes adversaires valides pour chaque match TL
    ...
    # Garde seulement les matchs où l'adversaire existe dans gen
    valid_idx = [i for i in range(len(thi_gen)) if len(adv_gen_frames[i])>0]

    if len(valid_idx) == 0:
        # Pas de données adversaires (ex: saison L3 avec 1 seul match sans stats adversaire)
        # On affiche quand même les données TL disponibles sans comparaison
        st.info("ℹ️ Les données adversaires ne sont pas encore disponibles pour cette saison. Seules les statistiques de Thionville Lusitanos sont affichées.")
        ...
        def safe_concat(frames, valid):
            parts = [frames[i] for i in valid if len(frames[i])>0]
            return pd.concat(parts, ignore_index=True) if parts else pd.DataFrame()
        adv_off_frames  = [_adv_rows(off, thi_gen.iloc[i]["Match"]) for i in range(len(thi_gen))]
        adv_def_frames  = [_adv_rows(defe, thi_gen.iloc[i]["Match"]) for i in range(len(thi_gen))]
        adv_pass_frames = [_adv_rows(passe, thi_gen.iloc[i]["Match"]) for i in range(len(thi_gen))]
        adv_idx_frames  = [_adv_rows(idx, thi_gen.iloc[i]["Match"]) for i in range(len(thi_gen))]
        adv_gen   = safe_concat(adv_gen_frames, list(range(len(thi_gen))))
        adv_off   = safe_concat(adv_off_frames, list(range(len(thi_gen))))
        adv_def   = safe_concat(adv_def_frames, list(range(len(thi_gen))))
        adv_pass  = safe_concat(adv_pass_frames, list(range(len(thi_gen))))
        adv_idx   = safe_concat(adv_idx_frames, list(range(len(thi_gen))))
        ...
        buts_adv_list= [adv_gen[adv_gen["Match"]==row["Match"]]["Goals"].values[0] if len(adv_gen[adv_gen["Match"]==row["Match"]])>0 else 0 for _,row in thi_gen.iterrows()]
